...
| ID | 不变量 | 验证方式 |
|---|---|---|
| **I-1** | `state.copilotToken` 写入点只在 `token-sentinel.ts` 内（**初次启动经由 `setupCopilotToken()` → `sentinel.bootstrap(initialToken)` 间接写入也算在内**） | `rg --glob 'packages/proxy/src/**' 'state\.copilotToken\s*='`，唯一文件应为 `lib/token-sentinel.ts`（验收范围限定在 `packages/proxy/src/`，排除 `test/` 与 mock 文件） |
